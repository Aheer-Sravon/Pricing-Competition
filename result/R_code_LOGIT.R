# =============================================================================
# LOGIT MODEL: Comprehensive Three-Panel Figure for Q1 Journal Publication
# =============================================================================
...
theme_publication <- theme_bw(base_size = 11, base_family = "serif") +
  theme(
...
p1_scatter <- ggplot(logit_long, aes(x = RPDI, y = Delta)) +
  ...
  annotate("text", x = -0.7, y = -0.3, label = "Delta = RPDI Line", 
           angle = 18, size = 3, color = "gray40", fontface = "italic") +
  # Theme
  theme_publication +
...
p2_shock <- ggplot(shock_impact_data, 
  ...
  # Annotations
  annotate("text", x = 0.6, y = 0.15, label = "Nash (Δ=0)", 
           size = 2.8, color = "black", hjust = 0) +
  annotate("text", x = 0.6, y = 1.15, label = "Monopoly (Δ=1)", 
...
p3_price <- ggplot() +
  ...
  scale_color_manual(values = c("Q-learning" = "#1B9E77", 
                                 ...
                                 "DDPG" = "#E7298A")) +
  scale_shape_manual(values = algo_shapes) +
  scale_y_continuous(limits = c(1.35, 2.0)) +
  # Labels
  labs(
  ...
  # Annotations
  annotate("segment", x = 4, xend = 4, y = 1.66, yend = 1.91,
           arrow = arrow(ends = "both", length = unit(0.1, "inches")),
           color = "gray50") +
  annotate("text", x = 4.3, y = 1.78, label = "Gap", 
           size = 2.8, color = "gray50", hjust = 0) +
  annotate("text", x = 2.5, y = 1.78, label = "Theoretical Nash", 
           size = 2.8, angle = -55, color = "black", fontface = "italic") +
  # Theme
  theme_publication +
  theme(
    legend.position = "none",
...
title <- ggdraw() + 
  draw_label(
    "LOGIT Model: Algorithmic Competition Analysis",
    fontface = 'bold',
    fontfamily = 'serif',
    size = 14,
    x = 0.5,
...
ggsave("LOGIT_comprehensive_figure.png", 
       plot = final_plot,
       width = 14, 
       height = 5,
       dpi = 600,
...
ggsave("LOGIT_comprehensive_figure.pdf", 
       plot = final_plot,
       width = 14, 
       height = 5,
       device = cairo_pdf)
...
# Display message
cat("LOGIT comprehensive figure saved successfully!\n")
cat("Files created: LOGIT_comprehensive_figure.png, LOGIT_comprehensive_figure.pdf\n")
