...
class ModifiedPSOAgent:
    def __init__(self, env, agent_id, num_particles=20, w=0.729, c1=1.494, c2=1.494, c3=1.494, price_min=0.0, price_max=2.0):
        self.env = env
        self.id = agent_id
        ...
        self.c1 = c1
        self.c2 = c2
        self.c3 = c3  # Coefficient for external attractor
        self.p_nash = env.P_N  # External attractor: Nash price
    ...
    def update(self, opp_price):
        ...
        r3 = np.random.uniform(0, 1, self.num_particles)
        self.velocities = (
            self.w * self.velocities +
            self.c1 * r1 * (self.p_best_positions - self.particles) +
            self.c2 * r2 * (self.g_best_position - self.particles) +
            self.c3 * r3 * (self.p_nash - self.particles)  # Added external attractor term
        )
