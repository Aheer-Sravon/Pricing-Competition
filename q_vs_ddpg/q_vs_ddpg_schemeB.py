...
import argparse
parser = argparse.ArgumentParser(prog="q_vs_ddpg")
...
def main():
    ...
    os.makedirs("./results/shock_b", exist_ok=True)
    ...
    for model in models:
        ...
        metrices_df.to_csv(f"./results/shock_b/per_round_metrices_{model}.csv", index=False)
    
    df = pd.DataFrame(data)
    df.to_csv("./results/shock_b/q_vs_ddpg_schemeB.csv", index=False)
    ...
    print("\n[Results saved to ./results/shock_b/q_vs_ddpg_schemeB.csv]")
