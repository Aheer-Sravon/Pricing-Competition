...
def run_simulation(model, seed, verbose=True):
    """Run a single simulation of PSO vs DQN."""
    np.random.seed(seed)
    ...
    for t in range(HORIZON):
        # PSO updates with DQN's last price
        ...
        if verbose and (t + 1) % 1000 == 0:
            ...
            print(f"Step {t+1}: PSO price ~{avg_p_pso:.3f}, DQN price ~{avg_p_dqn:.3f}")
    ...
    return avg_price_pso, avg_price_dqn, delta_pso, delta_dqn, rpdi_pso, rpdi_dqn
...
for model in models:
    ...
    for run in range(num_runs):
        seed = SEED + run
        ap_pso, ap_dqn, d_pso, d_dqn, r_pso, r_dqn = run_simulation(model, seed, verbose=False)
...
df_detailed.to_csv("results/pso_vs_dqn_2.csv", index=False)
print(f"\nResults saved to pso_vs_dqn_2.csv")
